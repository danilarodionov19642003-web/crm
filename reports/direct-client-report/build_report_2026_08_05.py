...
OUT = OUT_DIR / "Отчет_Яндекс_Директ_Спектр_Металла_29_июля-5_августа_2026.docx"
...
def make_campaign_table(doc):
    data = [
        ["Кампания", "Показы", "Переходы", "CTR", "Расход без НДС", "Действия"],
        ["Профнастил", "2 649", "297", "11,21%", "3 832,31 ₽", "8"],
        ["Металлочерепица", "830", "90", "10,84%", "1 705,32 ₽", "4"],
        ["Итого", "3 479", "387", "11,12%", "5 537,63 ₽", "12"],
    ]
    ...
    for r_idx, row in enumerate(data):
        for c_idx, value in enumerate(row):
            ...
            cell.vertical_alignment = WD_CELL_VERTICAL_ALIGNMENT.CENTER
            if r_idx == 0:
                set_cell_shading(cell, "D7D9DE" if c_idx == 5 else DARK)
                color, bold = (DARK if c_idx == 5 else WHITE), True
...
def add_conversion_table(doc):
    rows = [
        ...
        ["Клик по телефону", "8 - посетитель открыл набор номера"],
        ["Переход в WhatsApp", "2 - посетитель открыл диалог с компанией"],
        ["Итого в Директе", "12 целевых действий, а не двенадцать оплаченных заказов"],
    ]
...
def add_traffic_source_table(doc):
    rows = [
        ...
        ["Автотаргетинг", "291", "4 268,65 ₽", "77,1%"],
        ["Ключевые фразы", "96", "1 268,98 ₽", "22,9%"],
        ["Итого", "387", "5 537,63 ₽", "100%"],
    ]
...
def add_seo_table(doc):
    rows = [
        ["Направление спроса", "Фразы", "Клики", "Расход без НДС", "Что создать"],
        ["Цена и стоимость", "94", "141", "1 666,49 ₽", "Страницы цен за лист и м²"],
        ["Выбор и применение", "64", "101", "980,77 ₽", "Материалы для крыши, забора и фасада"],
        ["Цвета и покрытия", "32", "47", "649,27 ₽", "RAL, оцинковка и покрытия"],
        ["Размеры и расчёт", "28", "33", "238,79 ₽", "Размеры, ширина и калькулятор"],
        ["Профили и модели", "12", "24", "474,26 ₽", "С8-НС44, МП-20 и Монтеррей"],
    ]
    table = doc.add_table(rows=len(rows), cols=5)
    set_table_geometry(table, [1.32, .62, .68, 1.05, 2.83])
    set_repeat_table_header(table.rows[0])
    for r_idx, row in enumerate(rows):
        for c_idx, value in enumerate(row):
            cell = table.cell(r_idx, c_idx)
...
def add_budget_table(doc):
    rows = [
        ...
        ["1. Контрольный тест", "10 000 ₽", "Потрачено с НДС - 6 756 ₽\nОстаток - 3 244 ₽", "Дочищаем запросы и сравниваем цену качественного обращения"],
...
def add_header_footer(doc):
    for section in doc.sections:
        ...
        set_font(fp.add_run("Срез на 5 августа 2026  •  metallomsk.ru  •  mentori.tech"), size=7.8, color=MID)
        fp.add_run(" " * 8)
