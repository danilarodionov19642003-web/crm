-- A scheduling confirmation only records the selected date. The work-start
-- note belongs to the morning reminder for work scheduled today.
...
begin
  ...
  if TG_OP = 'INSERT' and NEW.slot_status = 'scheduled' then
    v_message := '📅 <b>ОТКЛИК ЗАПЛАНИРОВАН</b>' || E'\n'
      ...
      || '🗓 <b>Дата:</b> ' || to_char(NEW.scheduled_date, 'DD.MM.YYYY');
    ...
    v_message := '🔄 <b>ДАТА ОТКЛИКА ИЗМЕНЕНА</b>' || E'\n'
      || v_divider || E'\n\n'
      || '📋 <b>Анкета:</b> ' || v_anketa_label || E'\n'
      || '🗓 <b>Новая дата:</b> ' || to_char(NEW.scheduled_date, 'DD.MM.YYYY');
