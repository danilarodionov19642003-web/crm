...
assert.match(clientApp, /if \(!tariffs\.length \|\| !anketas\.length\)/);
...
assert.match(clientIndex, /Купить отзывы/);
assert.match(clientApp, /Добавить отзывы для другой анкеты/);
assert.doesNotMatch(clientApp, /Новая анкета|ФИО новой анкеты|Ссылка на профиль|Размер оплаты|50% сейчас|100% сейчас/);
assert.doesNotMatch(clientApp, /data-order-new-name|data-order-profile-url|name="ordPay"/);
assert.match(clientApp, /const pay_full = false/);
assert.match(clientApp, /is_new_anketa: false/);
assert.match(clientApp, /Одна анкета добавлена в заказ дважды/);
assert.match(clientApp, /Math\.min\(cart\.MAX_ITEMS, anketas\.length\)/);
assert.doesNotMatch(orderCart, /source\.fullOnly === true/);
